...
import { Upload, X, File as FileIcon, Trash2, CheckCircle2, AlertCircle, Image as ImageIcon } from "lucide-react";
...
export default function UploaderTestPage() {
  ...
  // Configuration State
  const [allowedTypes, setAllowedTypes] = useState("image/*");
  const [uploadFolder, setUploadFolder] = useState("photos");
  ...
  const uploadFile = async (fileItem: FileItem) => {
    try {
      ...
      setFiles((prev) =>
        prev.map((f) =>
          f.id === fileItem.id
            ? { ...f, status: "success", progress: 100, publicUrl }
            : f
        )
      );
